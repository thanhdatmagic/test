import React from 'react'
import {useLocation} from 'react-router-dom'

export default function Menu(checkURL) {
    console.log(checkURL.props=='/');
    console.log(checkURL.props);
    
    const dataMenu= [
        { 
            name:'Promotion'
        },
        { 
            name:'Flight Schedule'
        },
        { 
            name:'About us'
        },
        { 
            name:'Payment Guide'
        },
    ]
 
  return (
    <>
    {checkURL.props == '/' ? 
    < div className=" light grid grid-cols-3 gap-4  my-10 justify-items-center absolute ">
                <div className="logo justify-center flex space-x-1">
                        <p className='text-2xl font-bold cursor-pointer text-black '>Baycungban</p>
                        <img src="https://cdn.britannica.com/41/4041-004-D051B135/Flag-Vietnam.jpg" alt="vietnam" className='border-blue-600 w-6 h-6 border-2 rounded-full m-2' />
                        <img src="data:image/png;base64," alt="usa" className=' w-6 h-6 border-2 rounded-full m-2' />
                        
                </div>
                <div className="menu">
                        <ul className='grid justify-items-center grid grid-cols-4 mr-2'>
                            {dataMenu.map(m=>(
                                <>
                                <li className='m-1 font-semibold text-black cursor-pointer'>
                                    {m.name}
                                </li>
                                </>
                            ))}
                        </ul>
                </div>
                <div className="buttonbooking">
                    <button className='font-semibold text-white bg-blue-600 rounded-lg p-2.5'>Booking now</button>
                </div>
    </div> 
    : 
    < div className=" dark grid grid-cols-3 gap-4  my-10 justify-items-center bg-blue-600 p-[20px] mt-0 ">
                <div className="logo justify-center flex space-x-1">
                        <p className='text-2xl font-bold cursor-pointer text-white'>Baycungban</p>
                        <img src="https://cdn.britannica.com/41/4041-004-D051B135/Flag-Vietnam.jpg" alt="vietnam" className='border-blue-600 w-6 h-6 border-2 rounded-full m-2' />
                        <img src="data:image/png;base64," alt="usa" className=' w-6 h-6 border-2 rounded-full m-2' />
                        
                </div>
                <div className="menu">
                        <ul className='grid justify-items-center grid grid-cols-4 mr-2'>
                            {dataMenu.map(m=>(
                                <>
                                <li className='m-1 font-semibold text-white cursor-pointer'>
                                    {m.name}
                                </li>
                                </>
                            ))}
                        </ul>
                </div>
                <div className="buttonbooking">
                    <button className='font-semibold text-blue-600 bg-white rounded-lg p-2.5'>Booking now</button>
                </div>
    </div>
            
    
    
    }

        
    </>
  )
}
